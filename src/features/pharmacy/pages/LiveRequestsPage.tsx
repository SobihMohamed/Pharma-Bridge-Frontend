    ...
    <div className="pt-8 px-4 md:px-8 pb-12 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 mt-4">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Live Requests</h1>
          ...
          <p className="text-slate-500 text-sm">Real-time prescription requests appearing as they arrive via SignalR.</p>
        </div>
        <AnimatePresence>
          {!isEmpty && (
            <motion.button
              ...
              className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm w-fit"
            >
      ...
      {isEmpty && (
        <motion.div
          ...
          className="flex flex-col items-center justify-center py-28 text-center rounded-3xl bg-gradient-to-b from-teal-50/60 to-transparent"
        >
          {/* Radar Animation */}
          ...
          <h3 className="text-xl font-semibold text-slate-900 mb-2">Radar active</h3>
          <p className="text-slate-500 max-w-sm leading-relaxed">
            Listening for live prescription requests in your area. New requests will appear here instantly.
          </p>
          <div className="mt-6 flex items-center gap-2 px-4 py-2 bg-white text-teal-700 text-sm font-medium rounded-full border border-teal-200 shadow-sm">
          ...
          <AnimatePresence mode="popLayout">
            {paginatedRequests.map((req, i) => (
              <motion.article
                ...
                whileHover={{ y: -3 }}
                className="group bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-4 shadow-sm hover:shadow-lg hover:border-teal-200 transition-all relative overflow-hidden"
              >
                ...
                <div className="flex-1 space-y-3">
                  ...
                  {req.imageUrl ? (
                    <div
                      ...
                      className="w-full h-40 rounded-xl overflow-hidden border border-slate-100 bg-slate-50 flex items-center justify-center cursor-pointer relative"
                    >
                      <img
                        src={req.imageUrl}
                        alt="Prescription"
                    ...
                    <div className="w-full h-40 rounded-xl border border-dashed border-slate-200 bg-slate-50/50 flex flex-col items-center justify-center text-slate-400">
                      <Image className="w-8 h-8 mb-2 opacity-50" />
                      <span className="text-xs font-medium">No image provided</span>
                    </div>
                  )}

                  <h3 className="text-lg font-semibold text-slate-900 pt-1 flex items-center gap-1.5">
                    {req.medicineName || (
                  ...
                  {req.patientNotes && (
                    <div className="bg-teal-50/60 p-3 rounded-xl border border-teal-100">
                      <p className="text-sm text-slate-600 line-clamp-3">
                        <span className="font-semibold block mb-1 text-xs text-teal-800">Patient notes</span>
                        {req.patientNotes}
                      </p>
                    </div>
                  )}
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
                  <div className="text-xs font-semibold text-amber-700 bg-amber-50 px-2.5 py-1 rounded-full border border-amber-200">
